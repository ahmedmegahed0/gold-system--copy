...
import type { BarcodeInvoice } from '../../common/types/barcode-sales.types';
...
export const BarcodeInvoicesPage: React.FC = () => {
  ...
  const { invoices, fetchInvoices, isLoading, error, cancelBarcodeInvoice } = useBarcodeSales();
      ...
      <div className={viewingInvoice ? 'print:hidden' : ''}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1 mr-14">إدارة فواتير بيع الباركود وعرض تفاصيلها</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-100 bg-gray-50/30 px-6 py-4 gap-4">
            <div className="flex p-1 bg-gray-100/80 rounded-lg">
              <button onClick={() => setActiveTab('ACTIVE')} className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all ${activeTab === 'ACTIVE' ? 'bg-white text-charcoal shadow-sm' : 'text-gray-500 hover:text-charcoal'}`}>فواتير مكتملة</button>
              ...
              <table className={`w-full text-base ${isRtl ? 'text-right' : 'text-left'}`}>
                ...
                <tbody className="divide-y divide-gray-50">
                  {currentInvoices.map((inv, index) => {
                    const totalGoldWeight = inv.items?.reduce((sum, item) => sum + (item.weight || 0), 0) || 0;
                    return (
            ...
            <div className="flex flex-col items-center justify-center p-6 print:p-0">
              <div className="flex justify-between items-center w-full max-w-3xl mb-6 print:hidden gap-4">
                <div className="flex gap-3">
                  {viewingInvoice.status === 'ACTIVE' && user?.role === 'OWNER' && (
                    <>
                ...
                <table className="w-full mb-8 border-collapse border border-charcoal text-center text-sm font-bold">
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      <th className="border border-charcoal py-3 px-2">العيار</th>
                      <th className="border border-charcoal py-3 px-2">الوزن</th>
                      <th className="border border-charcoal py-3 px-2">السعر (ج.م)</th>
                    </tr>
                  </thead>
                  <tbody>
      ...
      <ModalOverlay isOpen={!!cancelConfirmInvoice} onClose={() => setCancelConfirmInvoice(null)} title="تأكيد إلغاء الفاتورة">
        <div className="space-y-6">
          ...
          <div className="flex gap-4 pt-4 border-t border-gray-100">
            <button onClick={async () => { if (cancelConfirmInvoice) { await cancelBarcodeInvoice(cancelConfirmInvoice._id); setCancelConfirmInvoice(null); setViewingInvoice(null); } }} className="flex-1 py-3.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl flex items-center justify-center gap-2"><XCircle size={18} /> نعم، إلغاء الفاتورة</button>
