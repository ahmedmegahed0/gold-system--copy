...
const EditInvoiceModal: React.FC<{ isOpen: boolean; onClose: () => void; invoice: BullionSale | null; onSubmit: (id: string, data: any) => Promise<void> }> = ({ isOpen, onClose, invoice, onSubmit }) => {
  const [items, setItems] = useState<{ bullionItem: string; quantity: number; title: string; goldPricePerGram: number; makingChargePerUnit: number; }[]>([]);
    ...
    if (invoice) {
      setItems(
        invoice.items.map((item) => ({
          bullionItem: typeof item.bullionItem === 'object' ? (item.bullionItem as any)._id : item.bullionItem,
          quantity: item.quantity,
          title: item.title,
          goldPricePerGram: item.goldPricePerGram,
          makingChargePerUnit: item.makingChargePerUnit,
        }))
      );
      setError('');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(invoice._id, {
        items: items.map((i) => ({ bullionItem: i.bullionItem, quantity: i.quantity, goldPricePerGram: i.goldPricePerGram, makingChargePerUnit: i.makingChargePerUnit })),
      });
      onClose();
    } catch (err: any) {
  ...
  return (
    <ModalOverlay isOpen={isOpen} onClose={onClose} title="تعديل فاتورة السبايك">
      <form onSubmit={handleSubmit} className="space-y-6">
        {error && <div className="flex items-center gap-2 p-3 bg-red-50 text-red-600 text-sm rounded-xl border border-red-100"><AlertCircle size={16} />{error}</div>}
        <div>
          <label className="block text-sm font-bold text-charcoal mb-3">تعديل الأصناف</label>
            ...
            {items.map((item, idx) => (
              <div key={idx} className="flex flex-col gap-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" onClick={() => removeItem(idx)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1">العدد</label>
                    <input type="number" min="1" value={item.quantity || ''} onChange={(e) => updateItemField(idx, 'quantity', parseInt(e.target.value) || 1)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-gold focus:border-gold font-bold text-center" dir="ltr" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1">سعر الجرام</label>
                    <input type="number" min="0" value={item.goldPricePerGram || ''} onChange={(e) => updateItemField(idx, 'goldPricePerGram', parseFloat(e.target.value) || 0)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-gold focus:border-gold font-bold text-center" dir="ltr" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1">مصنعية القطعة</label>
                    <input type="number" min="0" value={item.makingChargePerUnit || ''} onChange={(e) => updateItemField(idx, 'makingChargePerUnit', parseFloat(e.target.value) || 0)} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-gold focus:border-gold font-bold text-center" dir="ltr" />
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="flex gap-4 pt-4 mt-6 border-t border-gray-100">
          <button type="submit" disabled={saving} className="flex-1 py-3.5 bg-gold hover:bg-[#b59540] text-white font-bold rounded-xl shadow-sm hover:shadow-md disabled:opacity-50 flex items-center justify-center gap-2">{saving ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />} حفظ التعديلات</button>
...
export const BullionInvoicesPage: React.FC = () => {
  const { i18n } = useTranslation();
  const isRtl = i18n.language.startsWith('ar');
  const { user } = useAuth();
  
  const { invoices, fetchInvoices, isLoading, error, updateSale, cancelSale } = useBullionSales();
  
  const [activeTab, setActiveTab] = useState<'COMPLETED' | 'CANCELLED'>('COMPLETED');
  const [searchTerm, setSearchTerm] = useState('');
  const [viewingInvoice, setViewingInvoice] = useState<BullionSale | null>(null);
  const [editingInvoice, setEditingInvoice] = useState<BullionSale | null>(null);
  const [cancelConfirmInvoice, setCancelConfirmInvoice] = useState<BullionSale | null>(null);
    ...
    <div className="space-y-6 relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-charcoal flex items-center gap-3">
              <div className="p-2.5 rounded-xl bg-theme-sales/10 text-theme-sales"><FileText size={24} /></div>
              فواتير السبايك والجنيهات
            </h1>
            <p className="text-gray-400 text-sm mt-1 mr-14">إدارة فواتير بيع السبايك والجنيهات وعرض تفاصيلها</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-100 bg-gray-50/30 px-6 py-4 gap-4">
            <div className="flex p-1 bg-gray-100/80 rounded-lg">
              <button onClick={() => setActiveTab('COMPLETED')} className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all ${activeTab === 'COMPLETED' ? 'bg-white text-charcoal shadow-sm' : 'text-gray-500 hover:text-charcoal'}`}>فواتير مكتملة</button>
              <button onClick={() => setActiveTab('CANCELLED')} className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all ${activeTab === 'CANCELLED' ? 'bg-white text-charcoal shadow-sm' : 'text-gray-500 hover:text-charcoal'}`}>فواتير ملغاة</button>
            </div>
            <div className="relative w-full sm:w-64">
              <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="بحث برقم الفاتورة..." className={`w-full py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-gold focus:border-gold bg-white transition-all ${isRtl ? 'pl-4 pr-10' : 'pr-4 pl-10'}`} />
          ...
          <div className="overflow-x-auto">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-24 gap-3 text-gray-400"><Loader2 size={32} className="animate-spin text-gold" /><span className="font-medium text-sm">جاري التحميل...</span></div>
              ...
              <table className={`w-full text-base ${isRtl ? 'text-right' : 'text-left'}`}>
                ...
                <tbody className="divide-y divide-gray-50">
                  {currentInvoices.map((inv, index) => (
                    <tr key={inv._id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-[#f4f7f4]'} hover:bg-gold/[0.05] transition-colors group border-b border-gray-100 last:border-0`}>
                      ...
                      <td className="px-6 py-4"><span className="inline-block bg-amber-50 text-amber-700 px-3 py-1.5 rounded-lg border border-amber-100/50 font-bold text-sm" dir="ltr">{inv.totalGoldWeight.toFixed(2)}g</span></td>
                      <td className="px-6 py-4"><span className="inline-block bg-purple-50 text-purple-700 px-3 py-1.5 rounded-lg border border-purple-100/50 font-bold text-sm" dir="ltr">{inv.totalMakingCharges.toLocaleString()} ج.م</span></td>
                      <td className="px-6 py-4"><span className="inline-block bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg border border-emerald-100/50 font-black text-sm" dir="ltr">{inv.grandTotal.toLocaleString()} ج.م</span></td>
                      <td className="px-6 py-4">{inv.status === 'COMPLETED' ? <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-md text-sm font-bold"><CheckCircle2 size={14} /> مكتملة</span> : <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-theme-returns/10 text-theme-returns rounded-md text-sm font-bold"><XCircle size={14} /> ملغاة</span>}</td>
          ...
          {filteredInvoices.length > 0 && !isLoading && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50/30">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 border border-gray-200 rounded-lg text-gray-500 hover:bg-white disabled:opacity-50 transition-colors">{isRtl ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}</button>
                <div className="flex items-center gap-1">
                  {Array.from({ length: totalPages }).map((_, idx) => (
                    <button key={idx} onClick={() => setCurrentPage(idx + 1)} className={`w-8 h-8 rounded-lg text-sm font-bold transition-all ${currentPage === idx + 1 ? 'bg-theme-sales text-white' : 'text-gray-500 hover:bg-white border border-transparent'}`}>{idx + 1}</button>
                  ))}
                </div>
                <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 border border-gray-200 rounded-lg text-gray-500 hover:bg-white disabled:opacity-50 transition-colors">{isRtl ? <ChevronLeft size={18} /> : <ChevronRight size={18} />}</button>
              </div>
            </div>
          )}
        </div>
      ...
      <ModalOverlay isOpen={!!viewingInvoice} onClose={() => setViewingInvoice(null)} title="تفاصيل الفاتورة" printFriendly={true}>
        {viewingInvoice && (() => {
          ...
          const sellerName = (viewingInvoice.seller as any)?.fullName || '---';
          return (
            <div className="flex flex-col items-center justify-center p-6 print:p-0">
              <div className="flex justify-between items-center w-full max-w-3xl mb-6 print:hidden gap-4">
                <div className="flex gap-3">
                  {viewingInvoice.status === 'COMPLETED' && user?.role === 'OWNER' && (
                    <>
                      ...
                      <button onClick={() => { setViewingInvoice(null); setEditingInvoice(viewingInvoice); }} className="px-6 py-2.5 bg-white border border-gray-200 text-charcoal hover:bg-gray-50 font-bold rounded-xl transition-colors flex items-center gap-2"><Edit2 size={18} /> تعديل</button>
                    </>
                  )}
                </div>
              ...
              <div className="bg-white p-8 sm:p-12 shadow-xl border border-gray-200 max-w-3xl w-full text-charcoal print:shadow-none print:border-none print:p-8 print:pt-12 mx-auto min-h-[297mm]" dir="rtl">
                <InvoicePrintHeader title={`فاتورة مبيعات سبايك ${viewingInvoice.status === 'COMPLETED' ? '' : '(ملغاة)'}`} />
                <div className="border-2 border-blue-600 rounded-xl p-4 text-center mb-8 bg-blue-50/30"><span className="text-2xl font-black text-blue-800">العميل: {customerName}</span></div>
                ...
                <table className="w-full mb-8 border-collapse border border-charcoal text-center text-sm font-bold">
                  <thead>
                    <tr className="bg-gray-100">
                      <th className="border border-charcoal py-3 px-2">م</th>
                      <th className="border border-charcoal py-3 px-2">اسم الصنف</th>
                      <th className="border border-charcoal py-3 px-2">العدد</th>
                      <th className="border border-charcoal py-3 px-2">وزن القطعة (ج)</th>
                      <th className="border border-charcoal py-3 px-2">الوزن الإجمالي</th>
                      <th className="border border-charcoal py-3 px-2">سعر الجرام اليوم</th>
                      <th className="border border-charcoal py-3 px-2">السعر الكلي (ج.م)</th>
                    </tr>
                  </thead>
                  <tbody>
                ...
                <div className="flex justify-end mt-8">
                  <div className="border-2 border-charcoal rounded-xl p-4 w-72 bg-gray-50 space-y-2">
                    <div className="flex justify-between items-center text-sm font-bold text-gray-500"><span>إجمالي وزن الذهب:</span><span dir="ltr">{viewingInvoice.totalGoldWeight.toFixed(2)} g</span></div>
                    <div className="border-t border-gray-300 pt-2 flex justify-between items-center text-lg font-black mt-2"><span>الإجمالي الكلي:</span><span dir="ltr">{viewingInvoice.grandTotal.toLocaleString()}</span></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <EditInvoiceModal
        ...
        onSubmit={async (id, data) => { await updateSale(id, data); }}
      />
      ...
      <ModalOverlay isOpen={!!cancelConfirmInvoice} onClose={() => setCancelConfirmInvoice(null)} title="تأكيد إلغاء الفاتورة">
        <div className="space-y-6">
          ...
          <div className="flex gap-4 pt-4 border-t border-gray-100">
            <button onClick={async () => { if (cancelConfirmInvoice) { await cancelSale(cancelConfirmInvoice._id); setCancelConfirmInvoice(null); setViewingInvoice(null); } }} className="flex-1 py-3.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl flex items-center justify-center gap-2"><XCircle size={18} /> نعم، إلغاء الفاتورة</button>
